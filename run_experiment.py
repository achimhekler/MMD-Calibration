...
val_save_mmd = os.path.join(val_save_d, "mmd")
...
if args.save_outputs_type == "feature":
    ...
    mmds = load_pickle(os.path.join(val_save_mmd, "mmds", "mmd.pickle"))
    mmd_scores = []
    for layer in args.ood_scoring_layers_list:
        mmd_scores.append(mmds[layer])
...
DAC_calibrator = DAC(ood_values_num=args.ood_values_num, bounded = True)
optim_params = DAC_calibrator.optimize(
    val_outputs, val_labels, val_ood_scores, loss="mse"
)

#optimize MMD DAC
MMD_calibrator = DAC(ood_values_num=args.ood_values_num, bounded = False)
optim_params_mmd = MMD_calibrator.optimize(
    val_outputs, val_labels, mmd_scores, loss="mse"
)
...
val_calib_outputs_mmd = MMD_calibrator.calibrate_before_softmax(
    val_outputs, mmd_scores
)
...
if args.combination_method == "ETS":
    ...
    # MMD ets
    p_mmd = ets_calibrate(val_calib_outputs_mmd, val_labels, val_calib_outputs_mmd, args.num_classes, "mse")
    ...
    # MMD SPL
    SPL_DAC_frecal_mmd, p_mmd, label_mmd = get_spline_calib_func(val_calib_outputs_mmd, val_labels)
else:
...
if args.combination_method != "SPL":
    ...
    # with MMD
    test_ece_dict, test_nll, test_mse, test_accu = ece_eval_all(p_mmd, val_labels)
    test_ece_1_with_mmd = test_ece_dict["ece_1"]
else:
    ...
    test_ece_1_with_DAC = test_ece_dict["ece_1"]
    # with MMD
    test_ece_dict, test_nll, test_mse, test_accu = ece_eval_all_from_conf_acc(p_mmd, label_mmd)
    test_ece_1_with_mmd = test_ece_dict["ece_1"]
...
print(f"- {args.combination_method} + MMD: test_ece_1:", test_ece_1_with_mmd)
...
for k in test_data_type:
    ...
    print(k)
    test_save_mmd = os.path.join(test_save_d, "mmd")
    ...
    test_mmds = load_pickle(os.path.join(test_save_mmd, "mmds", "mmd.pickle"))
    ...
    if args.save_outputs_type == "feature":
        ...
        test_mmd_scores = []
        for layer in args.ood_scoring_layers_list:
            test_mmd_scores.append(test_mmds[layer])
    elif args.save_outputs_type == "ood_score_arr":
        print("\nLoading ood_score_arr on validation set...")
        test_ood_scores = []
        for layer in args.ood_scoring_layers_list:
    ...
    calib_outputs_eval_mmd = MMD_calibrator.calibrate_before_softmax(
        test_outputs, test_mmd_scores
    )
    if args.combination_method == "ETS":
        # inputs should be outputs before softmax
        ...
        # MMD ets
        p_eval_mmd = ets_calibrate(
            val_calib_outputs_mmd, val_labels, calib_outputs_eval_mmd, args.num_classes, "mse"
        )
    elif args.combination_method == "SPL":
        # inputs should be outputs before softmax
        ...
        # MMD SPL
        p_eval_mmd, label_eval_mmd = spline_calibrate(
            SPL_DAC_frecal_mmd, calib_outputs_eval_mmd, test_labels
        )
    else:
        raise NotImplementedError
    ...
    if args.combination_method != "SPL":
        ...
        test_ece_1_with_DAC = test_ece_dict["ece_1"]
        # with MMD
        test_ece_dict, test_nll, test_mse, test_accu = ece_eval_all(p_eval_mmd, test_labels)
        test_ece_1_with_mmd = test_ece_dict["ece_1"]
    else:
        # Spline only calibrates top 1 prediction, so use different func. to evaluate
        # without DAC
        ...
        test_ece_1_with_DAC = test_ece_dict["ece_1"]
        # with MMD
        test_ece_dict, test_nll, test_mse, test_accu = ece_eval_all_from_conf_acc(p_eval_mmd, label_eval_mmd)
        test_ece_1_with_mmd = test_ece_dict["ece_1"]

    print(f"- {args.combination_method} w/o DAC: test_ece_1:", test_ece_1_wo_DAC)
    print(f"- {args.combination_method} + DAC: test_ece_1:", test_ece_1_with_DAC)
    print(f"- {args.combination_method} + MMD: test_ece_1:", test_ece_1_with_mmd)
    results.append([k, test_ece_1_uncal, test_ece_1_wo_DAC, test_ece_1_with_DAC, test_ece_1_with_mmd])


names = ["Uncal.",f"{args.combination_method} w/o DAC", f"{args.combination_method} + DAC", f"{args.combination_method} + MMD"]
